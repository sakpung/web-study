...
namespace Leadtools.Medical.UserManagementDataAccessLayer.DataAccessAgent
{
   internal static class Extensions
   {
      ...
      public static bool IsColumnValid(this IDataReader reader, string columnName)
      {
         bool fieldExists = reader.GetSchemaTable().Columns.Contains(columnName);
         return fieldExists;
      }
   ...
   internal static class UserNameResolver
   {
      ...
      public static Tuple<string, string, string> ToDb(string userName, string userType)
      {
         ...
         {
            return new Tuple<string, string, string>(EncodeName(userName), userName, EncodeType(userType));
         }
      ...
      {
         var encodedName = Hashing.GetHashNString(name.ToLower(), UserNameMaxLength);
         return encodedName;
      }
      ...
      public static string DecodeType(string userType)
      {
         if(!string.IsNullOrEmpty(userType))
         {
