...
Namespace PrintToPACSDemo
   ...
   Private WithEvents _cmbUnit As System.Windows.Forms.ComboBox
...
'		 Me._cmbUnit.SelectedIndexChanged += New System.EventHandler(Me._cmbUnit_SelectedIndexChanged);
   ' 
   ' _lblUnit
   ' 
...
'		 Me._rdNative.Click += New System.EventHandler(Me._rdNative_Click);
   ' 
   ' _rdMemory
   ' 
...
'		 Me._rdMemory.Click += New System.EventHandler(Me._rdMemory_Click);
   ' 
   ' _rdFile
   ' 
...
'		 Me._rdFile.Click += New System.EventHandler(Me._rdFile_Click);
   ' 
   ' _gbFileOptions
   ' 
...
'		 Me._txtFileName.TextChanged += New System.EventHandler(Me._txtFileName_TextChanged);
   ' 
   ' _lblFileName
   ' 
...
'		 Me._btnLoad.Click += New System.EventHandler(Me._btnLoad_Click);
   ' 
   ' _btnSave
   ' 
...
'		 Me.Load += New System.EventHandler(Me.Template_Load);
...
#End Region
   ...
   Private Sub Template_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
   InitializeTemplate()
   End Sub
   ...
   Try
   Dim twnCap As TwainCapability = _twainSession.GetCapability(TwainCapabilityType.ImageFrames, TwainGetCapabilityMode.GetValues)
   ...
   End Select
   Catch ex As Exception
   Dim errorMsg As String = String.Format("Get TwainCapabilityType.ImageYResolution returns {0}", ex.Message)
   FrmMain.AddErrorToErrorList(errorMsg)
   ...
   Select Case twnCap.Information.ContainerType
      ...
      Dim itemValue As TwainCapabilityValue = CType(Convert.ToUInt16(item), TwainCapabilityValue)
      Dim currentValue As TwainCapabilityValue = CType(Convert.ToUInt16(current), TwainCapabilityValue)
      Select Case itemValue
         Case TwainCapabilityValue.TransferMechanismFile
         _rdFile.Enabled = True
   ...
   End Select
   Catch ex As Exception
   Dim errorMsg As String = String.Format("Get TwainCapabilityType.ImageTranserMechanism returns {0}", ex.Message)
   FrmMain.AddErrorToErrorList(errorMsg)
   ...
   Try
   MySetCapability(TwainCapabilityType.ImageTransferMechanism, TwainItemType.Uint16, CUShort(_transferMode))
   Catch
   Return
   End Try
   ...
   Case TwainTransferMechanism.File
      ...
      Try
      ' ICAP_IMAGEFILEFORMAT 
      MySetCapability(TwainCapabilityType.ImageImageFileFormat, TwainItemType.Uint16, CUShort(_formatValue(_cmbFileFormat.SelectedIndex)))
   ...
   Private Sub _cmbUnit_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles _cmbUnit.SelectedIndexChanged
