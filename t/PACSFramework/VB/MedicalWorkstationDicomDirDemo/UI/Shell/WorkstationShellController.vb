...
#Region "Methods"
         ...
         Public Sub Run()
            ...
            If IsUninstallMode() Then
               dbConfigured = IsDbComponentsConfigured(message)
            ...
            CheckPacsConfig()
            dbConfigured = IsDbComponentsConfigured(message)
            If (ConfigurationData.CheckDataAccessServices And Not dbConfigured) Then                
...
#Region "Methods"
         ...
         Private Sub RegisterServices()
            ...
            Try
              Dim storageDataAccess As IStorageDataAccessAgent = DataAccessFactory.GetInstance(New StorageDataAccessConfigurationView()).CreateDataAccessAgent(Of IStorageDataAccessAgent)()
            ...
            Try
              Dim userManagement As IUserManagementDataAccessAgent = DataAccessFactory.GetInstance(New UserManagementDataAccessConfigurationView()).CreateDataAccessAgent(Of IUserManagementDataAccessAgent)()
         ...
         Private Function RequestUserToConfigureDbSucess(ByVal missingDbComponents As String) As Boolean
              ...
              If DialogResult.Yes = result Then
                ...
                If File.Exists(dbManagerFileName) Then
                  ...
                  Dim isDbConfigured As Boolean = IsDbComponentsConfigured(missingDbComponents)
...
#End If
         ...
         Private Sub ConfigureServersAndClients(ByVal settings As IWorkstationSettings)
            ...
            If settings.ServerList.Count <> 0 Then
              For Each server As DicomAE In settings.ServerList
                If (Not ServerExists(server)) AndAlso (Not IsWorkstationServer(settings, server)) Then
                  ConfigurationData.PACS.Add(New ScpInfo(server.AE, server.IPAddress, server.Port, server.Timeout))
                End If
              Next server
            End If
            ...
            If (Not String.IsNullOrEmpty(settings.DefaultImageQuery)) Then
              ...
              If Nothing IsNot server Then
                ConfigurationData.DefaultQueryRetrieveServer = New ScpInfo(server.AE, server.IPAddress, server.Port, server.Timeout)
              End If
            End If
            ...
            If (Not String.IsNullOrEmpty(settings.DefaultStore)) Then
              ...
              If Nothing IsNot server Then
                ConfigurationData.DefaultStorageServer = New ScpInfo(server.AE, server.IPAddress, server.Port, server.Timeout)
              End If
            End If
        ...
        End Sub
         ...
         Private Shared Function IsDbComponentsConfigured(<System.Runtime.InteropServices.Out()> ByRef missingDbComponents As String) As Boolean
            Dim loggingConfig As LoggingDataAccessConfigurationView
            Dim storageConfig As StorageDataAccessConfigurationView
            Dim workstationConfig As WorkstationDataAccessConfigurationView
            Dim userManagConfig As UserManagementDataAccessConfigurationView


            missingDbComponents = String.Empty

            loggingConfig = New LoggingDataAccessConfigurationView()
            storageConfig = New StorageDataAccessConfigurationView()
            workstationConfig = New WorkstationDataAccessConfigurationView()
            userManagConfig = New UserManagementDataAccessConfigurationView()

            If (Not WorkstationUtils.IsDataAccessSettingsValid(loggingConfig.DataAccessSettingsSectionName)) Then
              missingDbComponents &= "DICOM Logging Database." & Constants.vbLf + Constants.vbLf
            End If

            If (Not WorkstationUtils.IsDataAccessSettingsValid(storageConfig.DataAccessSettingsSectionName)) Then
              missingDbComponents &= "DICOM Storage Database." & Constants.vbLf + Constants.vbLf
            End If

            If (Not WorkstationUtils.IsDataAccessSettingsValid(workstationConfig.DataAccessSettingsSectionName)) Then
              missingDbComponents &= "Workstation Database." & Constants.vbLf + Constants.vbLf
            End If

            If (Not WorkstationUtils.IsDataAccessSettingsValid(userManagConfig.DataAccessSettingsSectionName)) Then
              missingDbComponents &= "User Management Database." & Constants.vbLf + Constants.vbLf
            End If

            Return (missingDbComponents Is String.Empty)
         End Function
