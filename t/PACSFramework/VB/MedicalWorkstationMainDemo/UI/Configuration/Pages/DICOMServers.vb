            ...
            Else
                ...
                Try
                    ...
                    For Each server As ScpInfo In ConfigurationData.PACS
                        ...
                        rowIndex = DICOMServersDataGridView.Rows.Add(New Object() {server.AETitle, server.Address, server.Port, server.Timeout, server.Secure, defaultQueryretrieve, defaultStore})
        ...
        Private Sub grdvDICOMServers_CellValidated(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs)
            Try
                ...
                ElseIf e.ColumnIndex = TimeoutColumn.Index Then
                    scp.Timeout = Integer.Parse(validatingRow.Cells(TimeoutColumn.Name).EditedFormattedValue.ToString().Replace(",", ""))
                ElseIf e.ColumnIndex = SecureColumn.Index Then
                    ' scp.Secure = int.Parse(validatingRow.Cells[SecureColumn.Name].EditedFormattedValue.ToString().Replace(",", ""));
                    scp.Secure = Boolean.Parse(validatingRow.Cells(SecureColumn.Name).Value.ToString())
                End If
            Catch exception As Exception
                System.Diagnostics.Debug.Assert(False, exception.Message)
            Finally
            ...
            Try
                ...
                If (DICOMServersDataGridView.Columns(e.ColumnIndex) Is DefaultStoreServerColumn) OrElse (DICOMServersDataGridView.Columns(e.ColumnIndex) Is DefaultQueryRetrieveColumn) Then
                    ...
                    Else
                        ...
                        End If
                    End If
                ElseIf DICOMServersDataGridView.Columns(e.ColumnIndex) Is SecureColumn Then
                    Dim validatingRow As DataGridViewRow = DICOMServersDataGridView.Rows(e.RowIndex)
                    Dim scp As ScpInfo = CType(validatingRow.Tag, ScpInfo)

                    scp.Secure = CBool(DICOMServersDataGridView.Rows(e.RowIndex).Cells(e.ColumnIndex).EditedFormattedValue)
