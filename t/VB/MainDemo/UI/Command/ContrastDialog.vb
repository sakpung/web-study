...
Namespace MainDemo
   Partial Public Class ContrastDialog : Inherits Form
      ...
      Private _internalContrastValue As Integer
      ...
      Protected Overridable Function DoEffect(ByRef effectedImage As RasterImage) As Boolean
         Try
            Dim command As ChangeContrastCommand = New ChangeContrastCommand(_internalContrastValue)
      ...
      Private Sub ContrastDialog_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
         Try
            If Not _beforeViewer.Image Is Nothing Then
               _tsbtnFit.Checked = False
               _tsbtnNormal.Checked = True
            ...
            If (_internalContrastValue / 10) < _numValue.Minimum Then
               _internalContrastValue = CInt(_numValue.Minimum)
               Contrast = CInt(_numValue.Minimum)
            End If

            If (_internalContrastValue / 10) > _numValue.Maximum Then
               _internalContrastValue = CInt(_numValue.Maximum)
               Contrast = CInt(_numValue.Maximum)
            End If

            _numValue.Value = CInt(_internalContrastValue / 10)
         Catch ex As Exception
            Throw ex
         End Try
      End Sub
      ...
      Private Sub _num_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles _numValue.Leave
         Try
            ...
            _internalContrastValue = CInt(_numValue.Value) * 10
