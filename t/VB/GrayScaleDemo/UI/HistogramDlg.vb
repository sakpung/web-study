...
Partial Public Class HistogramDlg
   ...
   Private Sub panel1_MouseMove(sender As Object, e As MouseEventArgs) Handles _pnlHistogram.MouseMove
      Try
   ...
   Private Sub CheckBestClipping()
      If _histValues IsNot Nothing Then
         Dim len As Integer = _histValues.Length

         Dim bGmax As Long = 0
         Dim fGmax As Long = 0

         For i As Integer = 0 To CInt((0.05) * len) - 1
            bGmax = Math.Max(bGmax, _histValues(i))
         Next

         For i As Integer = CInt((0.05) * len) To len - 1
            fGmax = Math.Max(fGmax, _histValues(i))
         Next

         If fGmax <> 0 AndAlso (bGmax \ fGmax) > 10 Then
            _numClipping.Value = Math.Min(20, CInt(bGmax \ fGmax))
         End If
      End If
   End Sub

   Private Sub HistogramDlg_Load(sender As Object, e As EventArgs) Handles Me.Load
