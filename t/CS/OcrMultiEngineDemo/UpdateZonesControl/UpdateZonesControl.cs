...
namespace OcrMultiEngineDemo.UpdateZonesControl
{
   public partial class UpdateZonesControl : UserControl
   {
      ...
      public void Activate(IOcrEngine ocrEngine, IOcrPage ocrPage, TreeView tvZonesList, IOcrZoneCollection zones)
      {
         ...
         MyLanguage ml = new MyLanguage(String.Empty, "None", 0);
         _languageComboBox.Items.Add(ml);
         foreach (string friendlyName in friendlyNames)
         {
            ml = new MyLanguage(languagesDictionary[friendlyName], friendlyName, 0);
            _languageComboBox.Items.Add(ml);
         }
         ...
         else if (_ocrEngine.EngineType == OcrEngineType.OmniPage)
      ...
      private OcrZone CurrentZone
      {
         get
         {
            return _zones[(int)_tvZonesList.SelectedNode.Tag];
         }
         set
         {
            _zones[(int)_tvZonesList.SelectedNode.Tag] = value;
         }
      }

      public void UpdateUIState()
      {
         bool enabled = _tvZonesList.SelectedNode != null;

         bool enableOmr = (_typeComboBox.SelectedItem != null && ((OcrZoneType)_typeComboBox.SelectedItem == OcrZoneType.Omr));
         bool enableCharacterFilters = enabled && (_typeComboBox.SelectedItem != null && (OcrZoneType)_typeComboBox.SelectedItem != OcrZoneType.Graphic && (OcrZoneType)_typeComboBox.SelectedItem != OcrZoneType.Micr && (OcrZoneType)_typeComboBox.SelectedItem != OcrZoneType.Omr);
         ...
         // controls if the LEAD OCR engine is selected since they are not supported for this engine.
         if (_ocrEngine.EngineType == OcrEngineType.OmniPageArabic)
         {
      ...
      public void ZoneToControls(int index)
      {
         ...
         if (index != -1)
         {
            ...
            if ((zone.CharacterFilters & OcrZoneCharacterFilters.Uppercase) == OcrZoneCharacterFilters.Uppercase)
               _uppercaseCheckBox.Checked = true;
            else
               _uppercaseCheckBox.Checked = false;

            if ((zone.CharacterFilters & OcrZoneCharacterFilters.Lowercase) == OcrZoneCharacterFilters.Lowercase)
               _lowercaseCheckBox.Checked = true;
            else
               _lowercaseCheckBox.Checked = false;

            if ((zone.CharacterFilters & OcrZoneCharacterFilters.Punctuation) == OcrZoneCharacterFilters.Punctuation)
               _punctuationCheckBox.Checked = true;
            else
               _punctuationCheckBox.Checked = false;

            if ((zone.CharacterFilters & OcrZoneCharacterFilters.Miscellaneous) == OcrZoneCharacterFilters.Miscellaneous)
