...
BEGIN TRY
   BEGIN TRANSACTION
	  ...
	  UPDATE JobsTable
	  SET cFailedTime=@time, cStatus=@status,cFailedErrorID=@failedErrorId, cFailedMessage=@failedInfo, cAttempts = cAttempts+1, cMustAbort = 0, cLastUpdatedTime= GetUTCDate(), cJobMetadata=ISNULL(@jobMetadata, cJobMetadata)
	  WHERE cGuid = @guid 
...
BEGIN TRY
   BEGIN TRANSACTION
      ...
      SELECT TOP 1 @guid = cGuid FROM JobsTable WITH (HOLDLOCK XLOCK ROWLOCK) WHERE cStatus IN (''New'', ''Failed'') AND cAttempts < @attempts AND cMustAbort=0 AND cJobType = @jobType ORDER BY cAddedTime ASC
      UPDATE JobsTable Set cStatus = ''Queried'', cWorker=@machineName Where cGuid = @guid
...
BEGIN TRY
   BEGIN TRANSACTION
	  ...
	  UPDATE JobsTable
	  SET cMustAbort=1, cAbortReason=@reason, cStatus=''Aborted''
	  WHERE @guid = cGuid 
