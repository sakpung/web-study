...
namespace Serverless_Cloud_Services_API
{
   public static class ExtractBarcode
   {
      ...
      public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = "Recognition/ExtractBarcode")]HttpRequestMessage req, TraceWriter log, ExecutionContext context)
      {
         try
         {
            ...
            using (imageReturn.Stream)
            {
               ...
               var barcodeResults = recognitionEngine.ExtractBarcode(imageReturn.Stream, options, barcodeEngine, barcodeList.ToArray(), 1, true);
