...
namespace DicomDemo
{
   ...
   public class NewDatasetDlg : System.Windows.Forms.Form
   {
...
#else
      public DicomDataSetInitializeType Flags
      {
         get
         {
            DicomDataSetInitializeType flags = DicomDataSetInitializeType.ExplicitVRLittleEndian;

            if(radioButtonLE.Checked)
            {
               if(radioButtonExplicit.Checked)
                  flags |= DicomDataSetInitializeType.ExplicitVRLittleEndian;
               else
                  flags |= DicomDataSetInitializeType.ImplicitVRLittleEndian;
            }
            else
            {
               flags |= DicomDataSetInitializeType.ExplicitVRBigEndian;
            }
...
#if (LTV15_CONFIG)
         checkBoxAddMandatoryModulesOnly.Hide();
         checkBoxAddMandatoryElementsOnly.Hide();
#endif
      ...
      private void InitializeComponent( )
      {
         ...
         this.checkBoxAddMandatoryModulesOnly.Name = "checkBoxAddMandaryModulesOnly";
